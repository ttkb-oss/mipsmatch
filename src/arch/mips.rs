...
mod tests {
    ...
    fn bytes_conversion() {
        ...
        assert_eq!(BinFormat::BigEndian.to_canonical()(&BE_JR_RA_BYTES), 0x0800E003);
        assert_eq!(BinFormat::LittleEndian.to_canonical()(&LE_JR_RA_BYTES), 0x0800E003);
        assert_eq!(BinFormat::BigSwapped.to_canonical()(&BS_JR_RA_BYTES), 0x0800E003);
        assert_eq!(BinFormat::LittleSwapped.to_canonical()(&LS_JR_RA_BYTES), 0x0800E003);
    }

    #[test]
    fn test_determine_bin_fmt() {
        assert_eq!(determine_bin_fmt(&BE_JR_RA_BYTES), Some(BinFormat::BigEndian));
        assert_eq!(determine_bin_fmt(&LE_JR_RA_BYTES), Some(BinFormat::LittleEndian));
        assert_eq!(determine_bin_fmt(&BS_JR_RA_BYTES), Some(BinFormat::BigSwapped));
        assert_eq!(determine_bin_fmt(&LS_JR_RA_BYTES), Some(BinFormat::LittleSwapped));
